	...
	{% else %}
		<section id="results">
			{% if results %}				
				...
				<div class="single-result full box"><p>{{result.text}}</p><span>tweeted by <a href="http://twitter.com/{{result.screen_name}}/statuses/{{result.id}}">{{result.screen_name}}</a> and favorited by @{{result.favorited_by|join:", @"}}</span></div>
				{% endfor %}
			{% else %}
